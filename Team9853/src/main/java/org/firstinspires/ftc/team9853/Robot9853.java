...
//    CONSTANTS     //
    ...
    public static final double OPTICAL_LOW = .4;
...
//    METHODS       //
    ...
    public boolean driveWithHeading(double angle, double speedModifier, int targetHeading) {
        ...
        requiredRotation = Math.abs(headingDif) * .5 / 180 + .15;

        //if going clockwise, set requiredRotation clockwise (-)
        if (headingDif < 0) requiredRotation *= -1;

        if (Math.abs(headingDif) > GYRO_MIN_ANGLE && Math.abs(requiredRotation) > .15)
